...
class ViewController: UIViewController, ARSCNViewDelegate, ARSessionDelegate {
    ...
    override func viewDidLoad() {
        ...
        if #available(iOS 13.0, *) {
            ...
            let lookcircle: CGRect = CGRect(x:0, y: 0, width:200, height: 220)
            ...
            lookatView.center = CGPoint(x: 350, y:800)
            lookatView.tintColor = .systemBackground
        ...
        DispatchQueue.main.async {
            ...
            if self.blink == 0{
                if self.calibrationmode == 1{
                    ...
                    self.tmpX = Int(round(smoothEyeLookAtPositionX + self.phoneScreenPointSize.width / 2))
                }
                else{
                    self.tmpY_new = Int(self.ya * Double(round(smoothEyeLookAtPositionY + self.phoneScreenPointSize.height / 2)) + self.yb)
                    self.tmpX_new = Int(self.xa * Double(round(smoothEyeLookAtPositionX + self.phoneScreenPointSize.width / 2)) + self.xb)
                    self.tmpY = Int(round(smoothEyeLookAtPositionY + self.phoneScreenPointSize.height / 2))
                    self.tmpX = Int(round(smoothEyeLookAtPositionX + self.phoneScreenPointSize.width / 2))
                }
                self.blinkcount = 0
            ...
            self.lookAtPositionXLabel.text = String(self.tmpX)
            ...
            self.calibrationx.append(self.tmpX)
            ...
            if self.calibrationmode == 1 {
                ...
                if self.errordistances.count == 600{
                    ...
                    //print(m)
                    let aveXx = self.calibrationx.reduce(0, +) / m
                    let aveYx = self.calibrationrealx.reduce(0, +) / m
                    let sxyx = Float(zip(self.calibrationx.map{$0-aveXx}, self.calibrationrealx.map{$0-aveYx}).map(*).reduce(0, +) / m)
                    let sxx = Float(zip(self.calibrationx.map{$0-aveXx}, self.calibrationx.map{$0-aveXx}).map(*).reduce(0, +) / m)
                    let ax = sxyx / sxx
                    let bx = Float(aveYx) - Float(ax) * Float(aveXx)
                    self.xa = Double(ax)
                    self.xb = Double(bx)
                    
                    let aveXy = self.calibrationy.reduce(0, +) / m
                    let aveYy = self.calibrationrealy.reduce(0, +) / m
                    let sxyy = Float(zip(self.calibrationy.map{$0-aveXy}, self.calibrationrealy.map{$0-aveYy}).map(*).reduce(0, +) / m)
                    let sxy = Float(zip(self.calibrationy.map{$0-aveXy}, self.calibrationy.map{$0-aveXy}).map(*).reduce(0, +) / m)
                    let ay = sxyy / sxy
                    let by = Float(aveYy) - Float(ay) * Float(aveXy)
                    self.ya = Double(ay)
                    self.yb = Double(by)
            ...
            if self.calibrationmode == 0{
                ...
                if self.errordistances.count == 1000{
                    ...
                    if self.count == 7{
                        ...
                        print(self.afterx)
                        let m = self.calibrationx.count
                        let errY = zip(self.calibrationx, self.calibrationrealx).map(-)
                        print(errY.count)
                        let errY_new = zip(self.aftery, self.calibrationrealx).map(-)
